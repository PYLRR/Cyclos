...
class pub(Primitive):
    """class used to describe cryptographic primitive pub"""
    arity = 1

    # Creates a pub primitive with its argument
    def __init__(self, arguments=[], constructedtype=False):
        super().__init__("pub", arguments, constructedtype)
        if constructedtype:
            try:
                assert isinstance(arguments[0], Type)
            except Exception:
                raise ProtocolError("Constructed type pub does not have the right argument (type) : "
                                    + arguments[0].__str__())
        else:
            try:
                assert isinstance(arguments[0], Variable)
            except Exception:
                raise ProtocolError("pub does not have the right argument (variable) : "
                                    + arguments[0].__str__())
